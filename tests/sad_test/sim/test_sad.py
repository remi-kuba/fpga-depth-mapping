...
def stereo_match(left_file, right_file, kernel, max_offset):
    ...
    for y in range(kernel_half, h - kernel_half):           
        offset_calculations[y] = {}   
        for x in range(kernel_half, w - kernel_half):
            offset_calculations[y][x] = []
            best_offset = 0
            prev_sad = 65534
            ...
            for offset in range(max_offset+1):               
                ...
                for v in range(-kernel_half, kernel_half):
                    for u in range(-kernel_half, kernel_half):
                        ...
                        sad += sad_temp
                offset_calculations[y][x].append(hex(sad))
                if sad < prev_sad:
                    prev_sad = sad
                    best_offset = offset
            depth[y, x] = best_offset * offset_adjust
    return w, h, left_img, right_img, depth, offset_calculations
...
async def test_data_out(dut):
    w, h, left_img, right_img, depth, offsets = stereo_match("../../data/Tsukuba_L.png", "../../data/Tsukuba_R.png", KERNEL_WIDTH, OFFSET)
    ...
    while True:
        ...
        diffs_out = dut.diffs_out
        diffs_out = [hex(a.value) for a in dut.diffs_out]
        best_offset = int(dut.offset_out.value)

        # only pixels that we actually care about
        if vc_out in offsets and hc_out in offsets[vc_out]:
            diffs_expected = offsets[vc_out][hc_out]
            dut._log.info(f"Pixel: {(vc_out, hc_out)} with offset {best_offset}\n\t{diffs_out}\n\t{diffs_expected}\n\tGrey: {pixel}, wanted: {depth[vc_out][hc_out]}")
        result_img.putpixel((hc_out, vc_out), pixel)
        result_img.save("../../output/output.png", "PNG")
        
        await ClockCycles(dut.clk_in, 1, rising=False)
        dut.data_valid_in.value = 0
        await ClockCycles(dut.clk_in, OFFSET + 3, rising=False)
...
async def test_sad(dut):
    ...
    w, h, left_img, right_img, _, _ = stereo_match("../../data/Tsukuba_L.png", "../../data/Tsukuba_R.png", KERNEL_WIDTH, OFFSET)
    ...
    # for y in range(1, 3):
        for x in range(w):
            ...
            await ClockCycles(dut.clk_in, 2*OFFSET + 3, rising=False)
...
def sad_runner():
    ...
    parameters = {"KERNEL_WIDTH": KERNEL_WIDTH, "OFFSET": OFFSET} 
